...
short modo = 0; //indica modo estático o dinámico, comienza en estático
...
//Modo dinámico
void agregarNodoDinamico(short num, short nodo){ //insertarEnABB (2)
    if (nodo>=2048/3){
        outputPuertoLog(CODIGO_ESCRITURA_INVALIDA);
        return;
    }
    if (AREA_DE_MEMORIA[nodo] == VACIO && nodo==0){
        AREA_DE_MEMORIA[nodo] = num;
        outputPuertoLog(CODIGO_EXITO);
    }else if (AREA_DE_MEMORIA[nodo] == VACIO){
        AREA_DE_MEMORIA[nodo] = nodo;
        AREA_DE_MEMORIA[3*nodo] = num;
        outputPuertoLog(CODIGO_EXITO);
    }else{
        if(AREA_DE_MEMORIA[3*nodo]<num)
            agregarNodo(num, 2*(nodo+1));
        else if(AREA_DE_MEMORIA[3*nodo]>num)
            agregarNodo(num, 2*(nodo+1)-1);
        else
            outputPuertoLog(CODIGO_NODO_EXISTENTE);
    }
...
short calcularAlturaDinamico(short nodo){ //imprime el ABb, 1 mayor a menor, 0 menor a mayor (3)
    if (nodo <= 2048/3 && AREA_DE_MEMORIA[nodo] != VACIO){
        short altIzquierda = calcularAltura(3*(2*(nodo+1) - 1) + 1);
        short altDerecha = calcularAltura(3*(2*(nodo+1)) + 1);
        if (altDerecha > altIzquierda)
            return altDerecha;
        else
            return altIzquierda;
...
short calcularSumaDinamico(short nodo){ //Calcular suma (4)
    short suma = 0;
    if (nodo <= 2048/3 && AREA_DE_MEMORIA[nodo] != VACIO){
        suma = AREA_DE_MEMORIA[3*nodo] + calcularSuma(2*(nodo+1)) + calcularSuma(2*(nodo+1)-1);
    }
    return suma;
}

void imprimirArbolDinamico(short nodo, short orden){ //imprime el ABb, 1 mayor a menor, 0 menor a mayor (5)
    if (nodo < 2048 && AREA_DE_MEMORIA[nodo] != VACIO){
        if (orden == 1 ){
            imprimirArbol(3*(2*(nodo+1)),orden);
            outputPuertoSalida(AREA_DE_MEMORIA[nodo]);      
            imprimirArbol(3*(2*(nodo+1)-1),orden);
        }
        else if(orden == 0){
            imprimirArbol(3*(2*(nodo+1)-1),orden);
            outputPuertoSalida(AREA_DE_MEMORIA[nodo]);      
...
void imprimirMemoriaDinamico(short N){ //imprime el ABb, 1 mayor a menor, 0 menor a mayor (5)
    for (short i = 0; i < N*3; i=(i+1)*3){//modo estático, donde cada posición supone simular
        outputPuertoSalida(AREA_DE_MEMORIA[i]);   
    }
}
//END Modo dinámico
